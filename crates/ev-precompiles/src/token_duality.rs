...
impl TokenDualityPrecompile {
    ...
    /// calling `balance_incr`. The check reads the current balance from
    /// the account and validates the new total fits in U256.
    ///
    /// Note: `balance_incr` is assumed to be safe after our check passes,
    /// as Revm's internal implementation also uses saturating/checked math.
    fn add_balance(
        internals: &mut EvmInternals<'_>,
        addr: Address,
        amount: U256,
